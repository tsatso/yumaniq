    const CMG_BASE = "/demos/rast-cr/cmg/";

    async function fetchCmgIndex() {
      const res = await fetch(CMG_BASE + "index.json", { cache: "no-store" });
      if (!res.ok) throw new Error("Failed to load CMG index");
      return res.json();
    }

    async function loadCmgFile(file) {
      const res = await fetch(CMG_BASE + file, { cache: "no-store" });
      if (!res.ok) throw new Error(`Failed to load CMG: ${file}`);
      return res.json();
    }

    async function initCmgChooser() {
      const index = await fetchCmgIndex();

      const select = document.getElementById("cmgSelect"); // <select id="cmgSelect">
      select.innerHTML = "";
      ...
      // Optional: auto-load first
      if (index.items.length) {
        const cmg = await loadCmgFile(index.items[0].file);
        applyCmg(cmg); // your existing function that uses the CMG
      }

      select.addEventListener("change", async () => {
        const cmg = await loadCmgFile(select.value);
        applyCmg(cmg);
      });
    }
...
*/
    ...
    // IMPORTANT: set this to the folder where your CMGs + index.json live
    // Example if they are under /demos/rast-cr/cmg/ :
    const CMG_BASE_DIR = "./cmg"; // or "./data", "./cmgs", etc.
    const CMG_INDEX_URL = `${CMG_BASE_DIR}/index.json`;

    async function loadCmgFromServer(path) {
    ...
    async function initCmgSelect() {
      ...
      let index;
      try {
        const res = await fetch(CMG_INDEX_URL, { cache: "no-store" });
        if (!res.ok) throw new Error(`Index fetch failed (${res.status})`);
        index = await res.json();
      } catch (e) {
        console.error(e);
        setStatus(`Failed to load index.json at ${CMG_INDEX_URL}`);
        return;
      }

      // Support a few index.json shapes:
      // 1) { "files": ["a.json","b.json"] }
      // 2) ["a.json","b.json"]
      // 3) { "items": [{ "label":"...", "file":"..." }, ...] }
      let items = [];
      if (Array.isArray(index)) {
        items = index.map((f) => ({ label: f, file: f }));
      } else if (Array.isArray(index.files)) {
        items = index.files.map((f) => ({ label: f, file: f }));
      } else if (Array.isArray(index.items)) {
        items = index.items.map((it) => ({
          label: it.label || it.file || it.href,
          file: it.file || it.href
        }));
      } else {
        setStatus(`index.json format not recognized`);
        return;
      }

      sel.innerHTML = items
        .map((it, i) => `<option value="${it.file}">${it.label || `CMG ${i + 1}`}</option>`)
        .join("");

      sel.addEventListener("change", async () => {
        const file = sel.value;
        const url = file.startsWith("http") || file.startsWith("/") ? file : `${CMG_BASE_DIR}/${file}`;
        try {
          await loadCmgFromServer(url);
        } catch (e) {
          console.error(e);
      ...
      if (items.length > 0) {
        ...
        const url = first.startsWith("http") || first.startsWith("/") ? first : `${CMG_BASE_DIR}/${first}`;
        try {
          await loadCmgFromServer(url);
        } catch (e) {
          console.error(e);
    ...
    // Init
    renderCmgGraph(sampleCmg); // optional: keep as fallback
    initCmgSelect();           // NEW: server-driven dropdown
